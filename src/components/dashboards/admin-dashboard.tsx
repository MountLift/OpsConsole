...
export default async function AdminDashboard() {
  const [campaigns, payoutsPending, invoicesOutstanding, creatorCount] = await Promise.all([
  ...
  const stats = [
    ...
    { label: "Creators on roster", value: creatorCount },
  ...
  return (
    <div>
      ...
      <div className="card divide-y divide-line">
        {campaigns.length === 0 && <div className="p-4 text-sm text-muted">No active campaigns yet.</div>}
        {campaigns.map((campaign) => (
          <div key={campaign.id} className="table-row flex items-center justify-between px-4 py-3 text-sm">
            <div>
              <div className="font-medium">{campaign.name}</div>
              <div className="text-muted text-xs">{campaign.brand.name}</div>
            </div>
            <div className="text-lift font-mono">{money(Number(campaign.budget))}</div>
          </div>
        ))}
      </div>
    </div>
