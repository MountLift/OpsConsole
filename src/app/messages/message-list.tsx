  ...
  return <div className="space-y-5">
    {messages.map((message) => {
      ...
      return <div key={message.id}>
        ...
        <div className={`flex ${message.own ? "justify-end" : "justify-start"}`}><div className={`max-w-[80%] ${message.own ? "items-end" : "items-start"} flex flex-col`}><span className="mb-1 text-[10px] font-mono text-muted">{message.senderName}</span><div className={`rounded-2xl px-4 py-3 text-sm leading-relaxed ${message.own ? "rounded-br-sm bg-paper text-white" : "rounded-bl-sm bg-[#f0e4db] text-paper"}`}>{message.body}</div><MessageTimestamp value={message.createdAt} /></div></div>
      </div>;
    })}
  </div>;
}